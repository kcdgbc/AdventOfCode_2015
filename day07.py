...
for part in (1,2):
  ...
  while (len(working) > 0):
    ...
    for line in working:
      ...
      match = re.match(getop, line)
      if match: operator = match.group(1)
      else: operator = "ASSIGN"

      if (operator == "ASSIGN") | (operator == "NOT"):
        if operator == "ASSIGN": match = re.match(assign, line)
        else: match = re.match(thenot, line)
        lop = match.group(2)

        if match.group(1).isdigit(): rop1 = int(match.group(1))
        else: 
          if match.group(1) in wires: rop1 = wires[match.group(1)]
          else: noinput = True

        if (part == 2) & (lop == 'b'): rop1 = answer1
    
      else:
        match = re.match(therest, line)
        lop = match.group(4)
        ...
        if match.group(3).isdigit(): rop2 = int(match.group(3))
        else:
          if match.group(3) in wires: rop2 = wires[match.group(3)]
          else: noinput = True

      if not noinput:
        if operator == "ASSIGN": wires[lop] = rop1
        if operator == "NOT":    wires[lop] = (~rop1) & 0xffff
        if operator == "AND":    wires[lop] = (rop1 & rop2) & 0xffff
        if operator == "OR":     wires[lop] = (rop1 | rop2) & 0xffff
        if operator == "LSHIFT": wires[lop] = (rop1 << rop2) & 0xffff
        if operator == "RSHIFT": wires[lop] = (rop1 >> rop2) & 0xffff
        working.remove(line)

      else: noinput = False

  if part == 1: answer1 = wires['a']
  else: answer2 = wires['a']

print ("part1 a= "+str(answer1)) 
print ("part2 a= "+str(answer2)) 
